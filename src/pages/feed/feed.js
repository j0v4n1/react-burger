...
const Feed = () => {
  ...
  const checkOrderStatus = (status, className) => {
    return orders ? orders.map((order, index) => {
      return order.status === status && index <= 9 ? <li key={order._id} className={className}>
        {order.number}
      </li> : null;
    }) : null;
  };

  const readyOrders = checkOrderStatus('done', `text text_type_digits-default ${styles['feed__orders-ready-list-item']}`);
